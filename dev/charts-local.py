...
def create_or_switch_branch_base_repo():
    ...
    if not options.branch:
        ...
        logging.debug(
            f"No branch specified on commandline, working with current branch: {current_branch}"
        )
    else:
...
def clone_charts_locally(branch, charts):
    ...
    for chart in charts["charts"]:
        ...
        # manually skip private repos
        matches = ["cool", "appsuite"]

        if any(x in name for x in matches):
            logging.info(f"Chart name {name} is enterprise - skipping...")
            continue
        logging.debug(f"Working on {chart} / tag {tag} / repo {repository}")
        chart_local_path = charts_path + "/" + name
        if not options.match in name:
            logging.info(
                f"Chart name {name} does not match {options.match} - skipping..."
            )
            continue
        elif registry == "":
        ...
        if os.path.isdir(chart_local_path):
            logging.debug(
                f"Found pre-existing {chart_local_path} skipping clone/pull, but will still reference chart in Helmfile..."
            )
            ...
            if git_url in doublette_dict:
                logging.debug(
                    f"{chart} located at {git_url} is already checked out to {doublette_dict[git_url]}"
                )
    ...
    for child_helmfile in child_helmfiles:
                ...
                if chart_def_prefix in line:
                    for chart_ident in charts_dict:
                        if ".Values.charts." + chart_ident + ".name" in line:
                            logging.debug(
                                f"found match with {chart_ident} in {line.strip()}"
                            )
                            line = charts_dict[chart_ident]
                            if os.path.isdir(
                                line
                                + "/charts/"
                                + charts["charts"][chart_ident]["name"]
                            ):
                                line += (
                                    "/charts/" + charts["charts"][chart_ident]["name"]
                                )
                            elif not os.path.isdir(line):
                                sys.exit(
                                    f"! Did not find directory to reference in Helmfile: '{line}'"
                                )
                            line = (
                                chart_def_prefix
                                + line
                                + '" # replaced by local-dev script'
                                + "\n"
                            )
        ...
        if child_helmfile_updated:
            ...
            if os.path.isfile(child_helmfile_backup):
                logging.debug(
                    "backup {child_helmfile_backup} already exists, will not create a new one."
                )
            else:
...
def revert_the_helmfiles():
    ...
    for child_helmfile in child_helmfiles:
        ...
        if os.path.isfile(child_helmfile_backup):
            logging.debug(
                f"Reverting {child_helmfile} from backup {child_helmfile_backup}"
            )
...
if __name__ == "__main__":
    ...
    p.add(
        "--match",
        default="",
        help="Clone/pull only charts that contain the given string in their name.",
    )
    p.add(
        "--loglevel",
        env_var="LOGLEVEL",
    ...
    fileHandler = logging.FileHandler(
        "{0}/{1}.log".format(log_path, os.path.basename(__file__))
    )
