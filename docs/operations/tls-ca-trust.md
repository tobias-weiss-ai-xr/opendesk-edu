...
On July 29, 2026, we discovered that the GEANT wildcard TLS certificate for `*.home.opendesk-edu.org` had expired 8 days prior (2026-07-22). This certificate was used by 22 different services in the opendesk namespace.

Since Let's Encrypt HTTP01 validation cannot work with internal-only domains (*.home.opendesk-edu.org resolves to 192.168.3.201 — a private IP), we pivoted to using the existing **opendesk-ca** internal Certificate Authority.
...
All services under `*.home.opendesk-edu.org`:
- ILIAS (lms.home.opendesk-edu.org)
- Moodle (moodle.home.opendesk-edu.org)
- Bookstack (wiki.home.opendesk-edu.org)
- Draw.io (drawio.home.opendesk-edu.org)
- Excalidraw (excalidraw.home.opendesk-edu.org.uni-OpenDesk.de)
- Collabora (office.home.opendesk-edu.org)
- Jitsi (meet.home.opendesk-edu.org)
- Nextcloud (files.home.opendesk-edu.org)
- OpenProject (projects.home.opendesk-edu.org)
- SeaweedFS (objectstorage.home.opendesk-edu.org)
- SOGo (contacts.home.opendesk-edu.org)
- Self-Service Password (ssp.home.opendesk-edu.org)
- Stalwart Mail (mail.home.opendesk-edu.org)
- Etherpad ( pad.home.opendesk-edu.org)
- And 8 more...
...
### Option A: Automatic (Recommended for Domain-Joined Kubernetes Machines)

Windows machines joined to the Kubernetes domain should automatically receive the CA certificate via Group Policy. No user action required.
...
```bash
# Linux/macOS
curl -v https://lms.home.opendesk-edu.org 2>&1 | grep "subject=" || echo "Still trusted"
...
Contact the GEANT CA or Kubernetes network team to renew the wildcard certificate for `*.home.opendesk-edu.org`.

### Option 3: Split Domain Setup

- Use public domains (e.g., home.opendesk-edu.org resolves to public IP) for external access
...
- [Kubernetes Internal CA Documentation](internal-ca-docs.md)
